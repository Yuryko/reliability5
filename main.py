...
R = np.ones(len(Z))            # временная переменная для вычисления количества модулей в ЗИП
zip = np.zeros(len(Z))
#######################
...
n = 1
#R = 0
k = 0 # неснижаемый запас
# a = 2 * G_MAZH[9] * 10000
B = 1
''' 
слишком маленький зип
for x in range(len(R)):
    su_gamma = 0
    n = 1 # начнем 0
    while B >= 0.001:
        gamma = n + 2  # гамма в формуле
        su_gamma = np.exp(-a) * n * ((a ** gamma) / np.math.factorial(gamma))
        B = -math.log(1 - (1 / a) * (np.exp(-a) * n * ((a ** gamma) / np.math.factorial(gamma))))
        print ('n='), n
        n = n+1
        print ('B ['), n, ('] ='), B
...
# B = -math.log(e, 1 - (a**3)/((a**3) + (n-1)*(1+a**2)))
# print a
# print n
# print B
'''
for x in range(len(R)):
    a[x] = 2 * G_MAZH[x] * 10000
    n = 1 # начнем 0
    while R[x] >= 0.001:
        R[x] = -math.log(1 - a[x] ** (k+2) / (a[x] ** (k + 2) + (n - k) * (1 + a[x]) ** (k+1)))
        n = n + 1
        zip[x] = n
print ('R = '), R
print ('ZIP ='), zip
